...
def model(TEST=True, Comparison_with_PCA=True, model_name="Autoencoder", optimizer_selection="Adam",
        ...
        if model_name == "Autoencoder" or model_name == "BNAutoencoder":
            with tf.variable_scope("encoder"):
                ...
                with tf.variable_scope("output"):
                    encoder_output = tf.nn.leaky_relu(layer(fully_3, [64, 2], [2]))

            with tf.variable_scope("decoder"):
                with tf.variable_scope("fully1"):
                    fully_4 = tf.nn.leaky_relu(layer(encoder_output, [2, 64], [64]))
                with tf.variable_scope("fully2"):
    ...
    with JG_Graph.as_default():  # as_default()는 JG_Graph를 기본그래프로 설정한다.
        ...
        with tf.variable_scope("shared_variables", reuse=tf.AUTO_REUSE) as scope:
            with tf.name_scope("inference"):
                encoder_output, decoder_output = inference(x)
